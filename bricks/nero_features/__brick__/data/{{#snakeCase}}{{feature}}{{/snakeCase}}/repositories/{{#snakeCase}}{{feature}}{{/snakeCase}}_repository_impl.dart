...
import 'package:{{#snakeCase}}{{project_name}}{{/snakeCase}}/domain/{{#snakeCase}}{{feature}}{{/snakeCase}}/repositories/{{#snakeCase}}{{feature}}{{/snakeCase}}repository.dart';

const errorMsg = 'Something went wrong';

class {{#pascalCase}}{{feature}}{{/pascalCase}}RepositoryImpl implements {{#pascalCase}}{{feature}}{{/pascalCase}}Repository {
  {{#pascalCase}}{{feature}}{{/pascalCase}}RepositoryImpl(this.remoteDataSource, this.localDataSource);

  final {{#pascalCase}}{{feature}}{{/pascalCase}}RemoteDataSource remoteDataSource;

  {{#usecases}}
  @override
  Future<Either<Failure, {{#pascalCase}}{{name}}{{/pascalCase}}Entity>> {{method}}{{#pascalCase}}{{name}}{{/pascalCase}}({{#usecases.isHaveBody}}{{#pascalCase}}{{usecases.name}}{{/pascalCase}}Body params{{/usecases.isHaveBody}}) async {
    try {
      final remote{{#pascalCase}}{{name}}{{/pascalCase}} = await remoteDataSource.{{method}}{{#pascalCase}}{{name}}{{/pascalCase}}({{#isHaveBody}}{{#pascalCase}}{{name}}{{/pascalCase}}Body body{{/isHaveBody}});
